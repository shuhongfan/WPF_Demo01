...
namespace GameServer
{
    class GameTable
    {
        ...
        private System.Timers.Timer timer;      //用户定时产生棋子
        ...
        private void SetDot(int i, int j, int dotColor)
        {
            ...
            if (i == 0)
                k1 = k2 = 1;    //如果是首行，只需要判断下边的点
            else if (i == grid.GetUpperBound(0))
                k1 = k2 = grid.GetUpperBound(0) - 1;    //如果是最后一行，只需要判断上边的点
            else
            {
                //如果是中间的行，上下两边的点都要判断
                k1 = i - 1;
        ...
        public void UnsetDot(int i, int j, int color)
        {
            ...
            string str = string.Format("UnsetDot,{0},{1},{2},{2}",
